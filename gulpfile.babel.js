...
const paths = {
    client: {
        ...
        styles: [`${clientPath}/{app,components}/**/*.stylus`],
        mainStyle: `${clientPath}/app/app.stylus`,
...
gulp.task('inject', cb => {
    runSequence(['inject:js', 'inject:css', 'inject:stylus'], cb);
});
                ...
                .pipe(plugins.sort()),
            {
                ...
                transform: (filepath) => {
                    let newPath = filepath
                        ...
                        .replace(/_(.*).stylus/, (match, p1, offset, string) => p1)
                        .replace('.stylus', '');
...
gulp.task('watch', () => {
    ...
    plugins.watch(paths.client.styles, () => {  //['inject:stylus']
...
gulp.task('constant', function() {
  ...
  return plugins.ngConstant({
    name: 'adventurejs2App.constants',
    deps: [],
    wrap: true,
    stream: true,
